    ...
    <div className="app-shell">
      ...
      <main id="home">
        ...
        <section className="dashboard" id="dashboard">
          <div className="dashboard-heading"><div><div className="eyebrow"><span /> WORKSPACE</div><h2>Your link universe.</h2><p>One place to create, search and understand every short link.</p></div><button className="refresh-button" type="button" onClick={loadLinks} disabled={linksLoading}><Icon>↻</Icon>{linksLoading ? 'Syncing…' : 'Refresh data'}</button></div>
          <div className="workspace-toolbar"><div className="search-box"><Icon>⌕</Icon><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search links or destinations…" /></div><div className="filter-tabs"><button className={filter === 'all' ? 'selected' : ''} onClick={() => setFilter('all')}>All <span>{totalLinks}</span></button><button className={filter === 'active' ? 'selected' : ''} onClick={() => setFilter('active')}>Active <span>{activeLinks}</span></button><button className={filter === 'inactive' ? 'selected' : ''} onClick={() => setFilter('inactive')}>Inactive <span>{inactiveLinks + expiredLinks}</span></button></div></div>
          <div className="links-panel">
            ...
            {linksLoading ? <div className="empty-state"><div className="spinner" /><p>Syncing your workspace…</p></div> : visibleLinks.length === 0 ? <div className="empty-state"><div className="empty-icon">⌁</div><h3>{search ? 'No matches' : 'Your workspace is empty'}</h3><p>{search ? 'Try a different search term.' : 'Create a short link above to get started.'}</p></div> : <div className="link-list">{visibleLinks.map((link, index) => { const linkUrl = `${SHORTENER_ORIGIN}/${link.short_code}`; const expired = isExpired(link); const active = link.is_active && !expired; return <article className="link-row" key={link.id}><div className="link-index">{String(index + 1).padStart(2, '0')}</div><div className="link-symbol">↗</div><div className="link-info"><a href={linkUrl} target="_blank" rel="noreferrer">{linkUrl}</a><p title={link.destination_url}>{link.destination_url}</p></div><div className={`status ${active ? 'active' : expired ? 'expired' : 'inactive'}`}><span />{active ? 'Active' : expired ? 'Expired' : 'Inactive'}</div><div className="link-date">{formatDate(link.created_at)}</div><div className="row-actions"><button type="button" onClick={() => copyLink(link.short_code)}>{copied === link.short_code ? 'Copied' : 'Copy'}</button><button className="analytics-action" type="button" onClick={() => loadAnalytics(link.short_code)} disabled={analyticsLoading}>View analytics</button></div></article> })}</div>}
          </div>

          {analytics && <section className="analytics-detail" id="analytics"><div className="analytics-detail-head"><div><div className="eyebrow"><span /> LINK INTELLIGENCE</div><h3>Performance snapshot</h3><p>{SHORTENER_ORIGIN}/{analytics.short_code}</p></div><div className="analytics-updated">Updated {formatDateTime(new Date())}</div></div><div className="analytics-metric-grid"><div className="analytics-metric"><span>All time</span><strong>{analytics.total_clicks}</strong><small>Total clicks</small><div className="metric-line"><i style={{ width: `${Math.min(100, analytics.total_clicks * 10)}%` }} /></div></div><div className="analytics-metric"><span>Today</span><strong>{analytics.clicks_today}</strong><small>Since midnight</small><div className="metric-line"><i style={{ width: `${Math.min(100, analytics.clicks_today * 20)}%` }} /></div></div><div className="analytics-metric"><span>This week</span><strong>{analytics.clicks_this_week}</strong><small>Current week</small><div className="metric-line"><i style={{ width: `${Math.min(100, analytics.clicks_this_week * 12)}%` }} /></div></div><div className="analytics-metric"><span>This month</span><strong>{analytics.clicks_this_month}</strong><small>Current month</small><div className="metric-line"><i style={{ width: `${Math.min(100, analytics.clicks_this_month * 8)}%` }} /></div></div></div></section>}
        </section>

        <section className="capabilities" id="features"><div className="section-heading"><div className="eyebrow"><span /> DESIGNED FOR WHAT'S NEXT</div><h2>A short link is just the beginning.</h2><p>The interface is ready to grow with the backend — analytics, link controls, QR codes, custom domains and more.</p></div><div className="capability-grid"><article className="capability-card featured"><div className="cap-icon">◉</div><span>01 / INSIGHT</span><h3>Click intelligence</h3><p>Move from a simple counter to a complete picture of how your links perform.</p><div className="card-arrow">↗</div></article><article className="capability-card"><div className="cap-icon">⌁</div><span>02 / CONTROL</span><h3>Link lifecycle</h3><p>Expiration and activation controls keep shared links under your command.</p><div className="card-arrow">↗</div></article><article className="capability-card"><div className="cap-icon">⌗</div><span>03 / SCALE</span><h3>Link infrastructure</h3><p>Async FastAPI and PostgreSQL give the product a serious foundation.</p><div className="card-arrow">↗</div></article></div></section>
      </main>
