...
function App() {
  ...
  async function shortenUrl(event) {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/links`, {
        ...
        body: JSON.stringify({ destination_url: destinationUrl, short_code: shortCode.trim() }),
      })
  ...
  async function loadAnalytics() {
    ...
    setError('')
    try {
      const response = await fetch(`${API_BASE}/api/v1/links/${encodeURIComponent(result.short_code)}/analytics`)
      const data = await response.json()
  ...
  async function copyLink() {
    if (!result?.short_code) return
    const link = `${SHORTENER_ORIGIN}/${result.short_code}`
  ...
  return (
    <div className="app-shell">
      <header className="navbar">
        <div className="brand"><div className="brand-mark"><Icon>↗</Icon></div><span>Smart<span>Shortner</span></span></div>
        <nav className="nav-links"><a href="#home">Home</a><a href="#features">Features</a><a href="#analytics">Analytics</a></nav>
      ...
      <main id="home">
        <section className="hero">
          ...
          <form className="shortener-card" onSubmit={shortenUrl}>
            <div className="field-row">
              <div className="input-wrap main-input"><Icon>↗</Icon><input type="url" value={destinationUrl} onChange={(event) => setDestinationUrl(event.target.value)} placeholder="Paste your long URL here..." required /></div>
          ...
          {result && <section className="result-card" aria-live="polite">
            ...
            <div className="result-content"><span className="result-label">YOUR SHORT LINK</span><a href={shortLink} target="_blank" rel="noreferrer">{shortLink}</a><p title={result.destination_url}>→ {result.destination_url}</p></div>
        ...
        <section className="features" id="features">
          <div className="section-heading"><div className="eyebrow"><span /> WHY SMART SHORTNER</div><h2>More than a shorter URL.</h2><p>A clean foundation today, with room to grow into the link platform you actually want.</p></div>
          <div className="feature-grid"><article><div className="feature-number">01</div><h3>Custom links</h3><p>Choose memorable short codes instead of random-looking URLs.</p></article><article><div className="feature-number">02</div><h3>Click tracking</h3><p>Every redirect can record click events and request metadata.</p></article><article><div className="feature-number">03</div><h3>Built to scale</h3><p>Async FastAPI, PostgreSQL and a layered architecture give us a strong base.</p></article></div>
        </section>
      </main>
